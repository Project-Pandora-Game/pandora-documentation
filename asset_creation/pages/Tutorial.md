...
>
> Usually you will encounter "Automatic image" layer, which is the current recommended way to add images. There are, however, many older assets that use simple, low-level "Image" layer. This type allows for more fine-tuning, but generally has fewer tools and it is easy to make mistakes when using it.\
...
- selecting the Template and its Variant for this layer, so the engine knows how to (or not to) transform this layer when the character changes poses and how to layer this layer among other assets
...
If you want to know what more is possible, or like to learn by example, we recommend reading this section dedicated to [Looking at existing assets](Tutorial_Existing_assets.md).
...
>
> Changing identifier or category will also cause any item created from this asset to be deleted, as Pandora will no longer recognize it.
...
> [!TIP]
> The easiest way to align your asset images to the character is by making them in size in the size 1000 x 1500 pixels.\
> Handheld items are exception to this and should be created with size 1500 x 1500 pixels, shifted by 250 pixels to the left relative to character image.
...
> - Export with maximum compression (9), to save space for any future asset creators
...
>
> Note, that assets creators also leave behind `reusePolicy` in the `.asset.ts` file to signal how much they are happy with you reusing their asset for new one one - some are even "Free to use" - letting you copy the whole asset as a base for your new one, if you want to.
